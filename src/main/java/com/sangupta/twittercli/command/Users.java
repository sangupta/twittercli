...
@Command(name = "users", description = "Display profile info for given users")
public class Users extends TwitterCommand {
	...
	public void doCommand() {
		...
		WebRequest request = WebInvoker.getWebRequest("https://api.twitter.com/1.1/users/lookup.json?screen_name=" + userStr, WebRequestMethod.GET);
		user.signRequest(request);
		WebResponse response = WebInvoker.executeSilently(request);
		TwitterUserProfile[] profiles = GsonUtils.getGson(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES).fromJson(response.getContent(), TwitterUserProfile[].class);
